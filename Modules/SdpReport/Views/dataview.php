	...
	<table id="dg" class="easyui-datagrid" style="width:<?=!@$isMobile?'auto':'1024px'?>;height:500px" 		
		...
		<thead>
			<tr>
				...
				<th field="institution_name">Institution</th>
				<th field="expired_date">@Expired</th>
		...
		<div class="search-box">
			<table>
				...
				<tr>
					...
					<td>Exp. Date</td><td><?=easyui_datebox(array('id'=>'fdate','name'=>'fdate'))?> to <?=easyui_datebox(array('id'=>'ldate','name'=>'ldate'))?></td>
					<td>Status</td><td><?=easyui_combobox_option(array('label'=>'EXPIRATION_STATUS','id'=>'sexpiration_status','name'=>'sexpiration_status'))?></td>
					<td>
	...
	<div id="dlg" class="easyui-dialog" style="width:<?=@$isMobile?'auto':'600px'?>;height:440px;padding:10px 20px" closed="true" buttons="#dlg-buttons">
		<form id="fm" method="POST" enctype="multipart/form-data" novalidate>
			...
			<div class="fitem">
				...
				<?=easyui_textbox(array('id'=>'ffullname','name'=>'fullname','width'=>'170px','required'=>false,'readonly'=>true))?>&nbsp;
			...
			<div class="fitem">
				...
				<?=easyui_sdp(array('name'=>'sdp_id','required'=>true))?>
			</div>
			<div class="fitem">
				<label>Institution:</label>
				<?=easyui_textbox(array('name'=>'institution_name'))?>
			</div>
			<div class="fitem">
				<label>Expired Date:</label>
				<?=easyui_datebox(array('name'=>'expired_date','required'=>true))?>
			</div>
			<div class="fitem">
...
<script type="text/javascript">
	...
	function doSearch(){
		$('#dg').datagrid('load',{
			...
			ssdp_id: $('#ssdp_id').combobox('getValue'),
			sexpiration_status: $('#sexpiration_status').combobox('getValue'),
