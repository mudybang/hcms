...
namespace Modules\SdpReport\Controllers;
...
class SdpReport extends BaseController{
    protected $db;
    protected $perm;
    protected $table;
    protected $model;
    ...
    public function index(){
        ...
        return view("\Modules\SdpReport\Views\dataview",$data);
	}
    public function get_data(){
        ...
        if($this->request->getpost('ssdp_id'))$w.=" and e.sdp_id='".$this->request->getpost('ssdp_id')."'";
        if($this->request->getpost('sis_all')==0)$w.=" and e.active=1";
        if($this->request->getpost('sexpiration_status')){
            if($this->request->getpost('sexpiration_status')=='Available'){
                $w.=" and expired_date >= CURDATE()";
            }else{
                $w.=" and expired_date < CURDATE()";
            }
        }
        if($this->request->getpost('fdate')){
			if($this->request->getpost('fdate')&&$this->request->getpost('ldate')){
				$w.=" and (a.expired_date BETWEEN '".$this->request->getpost('fdate')."' AND '".$this->request->getpost('ldate')."')";
			}else{
				$w.=" and a.expired_date > '".$this->request->getpost('fdate')."'";
			}
		}
        ...
        $results = array();
        $sql="select a.*,s.title,eid_number,fullname,join_date,d.name department_name,g.name grade_name,j.title jobtitle,es.label employee_status,
        b.name branch_name,p.name project_name from ".$this->table." a
            LEFT JOIN sdps s ON a.sdp_id=s.id
        ...
        foreach($this->db->query($sqldata)->getResultArray() as $row){
            $row['ck']=$row['id'];
            $row['ic_download']=$row['attachment']&&file_exists(FCPATH.'/uploads/employees/sdp/'.$row['attachment'])?
                '<a target="_blank" href="'.base_url("uploads/employees/sdp/".$row['attachment']).'"><i class="fas fa-download"></i></a>':
                '--no-attachment--';
            $data[]=$row;
        }
    ...
    public function create(){
        ...
        if ($this->model->save($data) === false) {
            ...
            $upload=$this->uploadfile->single_upload($id,'sdp');
